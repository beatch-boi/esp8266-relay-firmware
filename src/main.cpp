// TODO: Logs (журнал событий) Work In Progress :)
// Maybe OTA? IDK
    ...
    if (millis() % 1000 == 0)
    {
        ...
        {   
            ...
            if (time_client.update()) // This may cause the unnessary amount of internet traffic consumption. Need to limit the amout of requests per some time period.
                ...
                epoch_secs = start_time + ((millis64() / 1000) - time_offset); // Better to use loal times becaus they do not consume internet traffic. ^^^
