...
document.getElementById("submit").addEventListener("click", function () {
    ...
    {
        alert("WiFi SSID слишклм длинный.\nМаксимальная длина SSID 32 символов.");
        return;
    }
    const wifi_password = document.getElementById("wifi-password").value;
    if (wifi_password.length > 32)
    {
        alert("WiFi пароль слишклм длинный.\nМаксимальная длина пароля 32 символов.");
        return;
    }
...
if (!!window.EventSource) {
    ...
    events.addEventListener("sync_time", function (_event) {
        ...
        if (date === "" || time === "")
            alert("Поля дата и время должны бытьзаполнены.");
        else {
            let data = {};
